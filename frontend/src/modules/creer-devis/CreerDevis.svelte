<script>
  ...
  class DevisProfileSelectField {
    // Largeur suffisante pour UUID/CUID et identifiants applicatifs similaires.
    static PROFILE_ID_MAX_LENGTH = 100;
    ...
    normalizeProfileId(value) {
      ...
      const id = value.trim();
      if (!id) return '';
      if (id.length > DevisProfileSelectField.PROFILE_ID_MAX_LENGTH) return '';
      return id;
    }
  ...
  $effect(() => {
    profileSelectField.setAvailableProfileIds(layoutProfiles.map((profile) => profile.id));
  });
